...
public class F1RecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.ui_home_item,parent,false));
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == LAYOUT_TIME){
            ...
            proImageHW(jsonHomeF1ImageList.getHeight(), AppInfo.APP_HOME_ITEM_PRO, holderImage.hItemImage1); // 比例缩小 并设置imageView 高
            Glide.with(context)
                    .load(jsonHomeF1ImageList.getSmallUrl())
            ...
            holderImage.hItemLin1.setOnClickListener(v -> {
                onItemClickListener.onItemClick(v,position,jsonHomeF1ImageList);
            });
            holderImage.hItemImage2.setOnClickListener(v -> {
                onItemClickListener.onItemDownLoadClick(v,position,jsonHomeF1ImageList);
            });
            holderImage.hItemImage3.setOnClickListener(v -> {
                onItemClickListener.onItemLikeClick(v,position,jsonHomeF1ImageList);
            });
